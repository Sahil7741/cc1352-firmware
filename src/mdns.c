...
static struct mdns_string_pair_t mdns_get_next_substring(const void *rawdata, size_t size,
							 size_t offset)
{
	int recursion;
	size_t length;
...
static int mdns_socket_setup_ipv6(int sock, const struct in6_addr *jaddr)
{
	...
	if (zsock_bind(sock, (struct sockaddr *)&sock_addr, sizeof(struct sockaddr_in6))) {
		return -1;
	}
...
static size_t mdns_string_table_find(struct mdns_string_table_t *string_table, const void *buffer,
				     size_t capacity, const char *str, size_t first_length,
				     size_t total_length)
{
	...
	for (istr = 0; istr < string_table->count; ++istr) {
		...
		while (offset < total_length) {
			...
			sub_string = mdns_get_next_substring(buffer, capacity,
							     sub_string.offset + sub_string.length);
...
static void mdns_string_table_add(struct mdns_string_table_t *string_table, size_t offset)
{
	...
	table_capacity = ARRAY_SIZE(string_table->offset);
...
static void *mdns_string_make(void *buffer, size_t capacity, void *data, const char *name,
			      size_t length, struct mdns_string_table_t *string_table)
{
	size_t last_pos = 0;
	...
	while (last_pos < length) {
		...
		ref_offset = mdns_string_table_find(string_table, buffer, capacity,
...
static int mdns_multiquery_send(int sock, const struct mdns_query_t *query, size_t count,
				void *buffer, size_t capacity, uint16_t query_id)
{
	...
	for (iq = 0; iq < count; ++iq) {
		// Name string
		data = mdns_string_make(buffer, capacity, data, query[iq].name, query[iq].length,
					0);
		if (!data) {
			return -1;
		}
...
static struct mdns_string_t mdns_string_extract(const void *buffer, size_t size, size_t *offset,
...
static struct mdns_string_t mdns_record_parse_ptr(const void *buffer, size_t size, size_t offset,
...
static bool mdns_answer_check(const void *buffer, size_t size, size_t *offset, size_t records,
	...
	for (i = 0; i < records; ++i) {
		...
		*offset += 10;
		...
		if (length <= (size - (*offset))) {
			...
			namestr = mdns_record_parse_ptr(buffer, size, record_offset, record_length,
...
static size_t mdns_query_recv_internal(int sock, struct in6_addr *addrptr, const char *query,
				       size_t query_len)
{
	int ret;
	...
	offset = MDNS_POINTER_DIFF(data, buffer);
	if (mdns_answer_check(buffer, data_size, &offset, answer_rrs, query, query_len)) {
